...
(function()
{
    ...
    VS.ui.SearchBox = Backbone.View.extend({
        ...
        disableFacets : function(keepView)
        {
            ...
            {
                if (view && view != keepView && (view.modes.editing == 'is' || view.modes.selected == 'is')) {
                    view.disableEdit();
                }
            });
        ...
        focusNextFacet : function(currentView, direction, options)
        {
            ...
            if (!options.skipToFacet) {
                ...
                if (options.skipToFacet && currentView.type == 'text' && viewCount == viewPosition && direction >= 0) {
        ...
        {
            if ($(e.target).is('.VS-search-box') || $(e.target).is('.VS-search-inner') || e.type == 'keydown') {
                this.focusSearch(e);
            }
        },
    ...
    VS.ui.SearchFacet = Backbone.View.extend({
        ...
        searchAutocomplete : function(e)
        {
            ...
            if (autocomplete) {
                ...
                menu.outerWidth(Math.max(menu.width('').outerWidth(), autocomplete.element.outerWidth()));
            }
        },
                    ...
                    if (key == 'right') {
                        ...
                        if (VS.app.hotkeys.shift && key == 'tab') {
                            ...
                            if (key == 'tab') {
                                ...
                                if (VS.app.hotkeys.command && (e.which == 97 || e.which == 65)) {
                                    e.preventDefault();
                                    this.options.app.searchBox.selectAllFacets();
                                    return false;
                                } else {
                                    if (VS.app.hotkeys.printable(e) && this.modes.selected == 'is') {
                                        this.options.app.searchBox.focusNextFacet(this, -1, {startAtEnd: -1});
                                        this.remove(e);
                                    } else {
                                        if (key == 'backspace') {
    ...
    VS.ui.SearchInput = Backbone.View.extend({
        ...
        autocompleteValues : function(req, resp)
        {
            ...
            this.app.options.callbacks.facetMatches(searchTerm, function(prefixes)
            {
                ...
                resp(_.sortBy(matches, function(match)
                {
                    if (match.label) return match.category + '-' + match.label; else             return match;
                }));
            });
    ...
    VS.app.hotkeys = {
        ...
        printable : function(e)
        {
            ...
            if (e.type == 'keydown') {
                ...
                if ((code >= 32 && code <= 126) || (code >= 160 && code <= 500) || (String.fromCharCode(code) == ":")) {
                    return true;
                }
            }
            return false;
    ...
    $.fn.extend({
        ...
        autoGrowInput: function()
        {
            return this.each(function()
            {
                ...
                // a single keypress.
                var events = 'keydown.autogrow keypress.autogrow ' + 'resize.autogrow change.autogrow';
