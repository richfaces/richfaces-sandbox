    ...
    jQuery.extend({
        ...
        keyTips: function (options) {
            ...
            $(function () {
                ...
                requiresHighlighting = !/opera/.test(navigator.userAgent);
                ...
                var escapeId = function (id) {
                    ...
                    return result;
                        ...
                        getForAttributeValue = function(label) {
                            return $(label).attr("htmlFor") || $(label).attr("for");
                        },

                        getFieldAccessKey = function (label) {
                            var relatedFieldId = escapeId("#" + getForAttributeValue(label)),

                                    accessKey = $.trim(relatedFieldId !== "" ? $(relatedFieldId + "[accesskey]").attr("accesskey") || $(label)
                                            .attr("accesskey") : $(label).attr("accesskey"));
                        ...
                        createPopup = function (field, accessKey) {
                            ...
                            var popup = $("<div/>").text(accessKey).css("left", popupLocation.left).css("top", popupLocation.top).addClass(settings.popupClass)
                                    .appendTo("body").get(0);
                        ...
                        insertAccessKeyTags = function () {
                            ...
                            $("label[for], label[accesskey]").each(function () {
                                ...
                                var accessKeyMarkup = "<" + settings.accessKeyTag + (settings.accessKeyTagClass != "" ? " class='" + settings.accessKeyTagClass
                                        + "'" : "") + ">" + labelHtml.substr(accessKeyIndex, 1) + "</" + settings.accessKeyTag + ">",
                        ...
                        createPopups = function () {
                            ...
                            if (settings.highlightMode == "popup") {
                                ...
                                $("label > " + settings.accessKeyTag).each(function () {
                                    var text = $(this).text(),
                                            label = $(this).parent(),
                                            relatedFieldId = escapeId("#" + getForAttributeValue(label)),
                        ...
                        doHighlightAccessKeys = function (highlight) {
                            ...
                            if (settings.highlightMode === "popup") {
                                ...
                                $.each(accessKeyPopups, function () {
                                    ...
                                    i++;
                                    var popupLocation = getPopupLocation(field);
                                    $(this).css("left", popupLocation.left).css("top", popupLocation.top).toggle(highlight);
                                });
                            } else if (settings.highlightMode === "toggleClass") {
                                /* Toggle label class*/
                                $("label > " + settings.accessKeyTag + "." + settings.accessKeyTagClass).each(function () {
                                    var text = $(this).text(),
                                            label = $(this).parent(),
                                            relatedFieldId = escapeId("#" + getForAttributeValue(label)),
                                            accessKey = getFieldAccessKey(label);
                ...
                /* bind handlers*/
                $(document).bind("keydown.keytips",
                        function (e) {
                            $.trace("KeyTips document.keyDown: keyCode=" + e.keyCode + ", altKey=" + e.altKey + ", shiftKey=" + e.shiftKey, settings.debug);
                            if (!accessKeysHighlighted && (
                                    (e.keyCode == 18 && !requiresShiftAlt) || (e.keyCode == 16 && e.altKey && requiresShiftAlt) || (e.keyCode == 18 && e
                                            .shiftKey && requiresShiftAlt))) {
                                /* Highlight all the access keys*/
                                highlightAccessKeys();
                                /*accessKeysHighlighted = true;*/
                            }
                        }).bind("keyup.keytips", function (e) {
                            $.trace("KeyTips document.keyUp: keyCode=" + e.keyCode + ", altKey=" + e.altKey + ", shiftKey=" + e.shiftKey, settings.debug);
